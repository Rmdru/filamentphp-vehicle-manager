...
class BelgiumFuelPriceService
{
    ...
    private function crawl(): array
    {
        try {
            ...
            if (! $response->ok()) {
                Log::channel('fuel-prices')->warning('Belgium fuel prices HTTP response not OK', [
                    'status' => $response->status(),
                    'url' => config('belgium_fuel_prices.crawl_url'),
                ]);
                return [];
            }
            ...
            Log::channel('fuel-prices')->error('Belgium fuel prices crawling failed', [

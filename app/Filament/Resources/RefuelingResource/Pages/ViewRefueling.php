                ...
                Section::make('cirumstances')
                    ...
                    ->schema([
                        TextEntry::make('tires')
                            ->badge()
                            ->label(__('Tires'))
                            ->color(fn(string $state): string => RefuelingTires::from($state)->getColor())
                            ->icon(fn(string $state): string => RefuelingTires::from($state)->getIcon())
                        ...
                        TextEntry::make('climate_control')
                            ...
                            ->color(fn(string $state): string => RefuelingClimateControl::from($state)->getColor())
                            ->icon(fn(string $state): string => RefuelingClimateControl::from($state)->getIcon())
                            ->formatStateUsing(fn(string $state) => RefuelingClimateControl::from($state)->getLabel())
                            ->hidden(fn($state) => empty($state)),
                        TextEntry::make('routes')
                            ->badge()
                            ->label(__('Routes'))
                            ->color(fn(string $state): string => RefuelingRoutes::from($state)->getColor())
                            ->icon(fn(string $state): string => RefuelingRoutes::from($state)->getIcon())
                            ->formatStateUsing(fn(string $state) => RefuelingRoutes::from($state)->getLabel())
                            ->hidden(fn($state) => empty($state)),
                        TextEntry::make('driving_style')
                            ->badge()
                            ->label(__('Driving style'))
                            ->color(fn(string $state): string => RefuelingDrivingStyle::from($state)->getColor())
                            ->icon(fn(string $state): string => RefuelingDrivingStyle::from($state)->getIcon())
                ...
                Section::make('payment')
                    ...
                    ->schema([
                        TextEntry::make('payment_method')
                            ...
                            ->icon(fn(string $state): string => RefuelingPaymentMethod::from($state)->getIcon())
